...
let varCreatedBy = document.querySelector("#createdBy");
varCreatedBy.innerHTML += " Rodrigo Figari";
...
// ----------------------------------  ACA CAMBIE EL TRUE POR "LO DIFERENTE A LO QUE TENGAS" -----------------------
// EN CASO DE TENER FALSE PONELO EN TRUE, EN CASO DE TENER TRUE PONELO A FALSE, ESTO NOS SIRVE POR SI CLICKEAMOS UNA TAREA DE MANERA EQUIVOCA Y QUEREMOS VOLVER A PONERLA COMO IMCOMPLETA
// Tu código acá:
...
// HICE EL EXTRA (CHECKBOX)
function buildToDo(todo, index) {
  // Tu código acá:
  //1,2
  let toDoShell = document.createElement("div");
  toDoShell.className = "toDoShell";
  // toDoShell.setAttribute("class","toDoShell") // OTRA ALTERNATIVA
  ...
  //6
  // todo.complete ? toDoText.className = "completeText" : null // OTRA ALTERNATIVA
...
*/
  ...
  // checkbox.onclick = completeToDo   // OTRA ALTERNATIVA
  ...
  //3 de la funcion completeTodo
  // toDoText.onclick = completeToDo // OTRA ALTERNATIVA
...
function buildToDos(toDos) {
  ...
  let mapeo = toDos.map(buildToDo);
  return mapeo;

  // let mapeo = toDos.map(elemento => {return buildToDo(elemento)})
  // return mapeo

  // let mapeo1 = toDos.map(elemento => buildToDo(elemento))
  // return mapeo1

  // return toDos.map(elemento => buildToDo(elemento))

  // return toDos.map(function(elemento){
  //   buildToDo(elemento)
  // })

  // return toDos.map(function(elemento){
  //   return buildToDo(elemento)
  // })
}
...
// ------------------------------------ AGREGAMOS UNA CONDICION PARA QUE NO ME CREE OBJETOS "TAREAS VACIAS" ----------------------------
function addToDo() {
  // Tu código acá:
  //1 < Hagregamos una condicion para que no cree objetos con string vacio ("") >
  if (document.getElementById("toDoInput").value === "") {
    alert("Ingresar una tarea por hacer");
  } else {
    let nuevoToDo = new ToDo(document.getElementById("toDoInput").value);
    //2
    toDoItems.push(nuevoToDo);
    //3
    document.getElementById("toDoInput").value = ""; // no puedo seteralo el valor a la variable nuevoToDo ya que no estaria haciendo sobre el elemento input "toDoInput"
  }
  //4
  displayToDos();
}
...
// ----------------------------------------- AGREGAMOS LA OPCION DE PODER TOCAR ENTER EN VEZ DEL BOTON AGREGAR -------------------------  ESTAN HABILITADAS LAS DOS OPCIONES
// Tecla enter
function enter(e) {
  if (e.keyCode === 13) {
    addToDo();
...
// Opcion metodo .Onclick >> pasamos funcion sin parentesis para que no se ejecute al cargarse la pag, sino al accionarse el evento, osea, hacer click en AGREGAR
...
function completeToDo(event) {
  ...
  const index = event.target.id; // obtenemos el id del elemento que acciono el evento
  //1
  toDoItems[index].completeToDo(); // seteamos en TRUE al atributo this.complete del objeto toDo >> Como completetoDO es un metodo de la clase toDo, primero seleccionamos el objeto que creamos con la clase y a ese objeto le invocamos el metodo completeTodo()
  //2
  displayToDos(); // actualizamos los elementos pintados
}
